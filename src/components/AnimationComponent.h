...
public:
	...
	void move(int x, int y, int speed, std::tr1::function<void()> callback = nullptr_t());
	void fadeIn(int time, std::tr1::function<void()> callback = nullptr_t());
	void fadeOut(int time, std::tr1::function<void()> callback = nullptr_t());
